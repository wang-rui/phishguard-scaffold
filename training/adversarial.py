    ...
    try:
        ...
        with torch.no_grad():
            logits_clean = safe_get_logits(
                model, input_ids=inputs["input_ids"], attention_mask=attention_mask
            )
        ...
        for step in range(steps):
            ...
            try:
                ...
                logits_pert = (
                    logits_clean + torch.randn_like(logits_clean) * epsilon * 0.1
                )

            # Compute adversarial loss (maximize distribution difference)
            if attack_type == "js":
                adv_loss = -js_divergence_with_logits(
                    logits_clean, logits_pert, temperature
                )
            else:
                adv_loss = -kl_divergence_with_logits(
                    logits_clean, logits_pert, temperature
                )
...
def compute_adversarial_loss(model, inputs, cfg: Dict) -> torch.Tensor:
    ...
    try:
        ...
        try:
            delta = adversarial_perturbation(
                model, inputs, epsilon, steps, "pgd", temperature
            )
            delta = safe_device_transfer(delta, device)
        except Exception as e:
            logger.warning(
                f"Adversarial perturbation generation failed: {e}. Skipping adversarial loss."
            )
        ...
        try:
            ...
            logger.warning(
                f"Adversarial forward pass failed: {e}. Using noise-based approximation."
            )
        ...
        raise ValueError(
            f"Adversarial loss requires configuration keys: adv_eps, adv_steps. Missing: {e}"
        )
    except Exception as e:
...
def compute_adversarial_loss_detailed(
    model, inputs, cfg: Dict

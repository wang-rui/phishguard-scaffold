...
def safe_model_forward(
    ...
    # Prepare kwargs
    kwargs = {"attention_mask": attention_mask, "return_dict": return_dict}
...
def safe_device_transfer(tensor, device):
    ...
    try:
        ...
        logger.warning(
            f"Device transfer failed: {e}. Keeping tensor on original device."
        )
        return tensor
...
def validate_model_inputs(input_ids=None, inputs_embeds=None, attention_mask=None):
    ...
    if inputs_embeds is not None and attention_mask is not None:
        if inputs_embeds.shape[:2] != attention_mask.shape[:2]:
            raise ValueError(
                f"inputs_embeds shape {inputs_embeds.shape} doesn't match attention_mask shape {attention_mask.shape}"
            )
...
def debug_model_structure(model, name="model"):
    ...
    logger.info(
        f"Has 'get_input_embeddings' method: {hasattr(model, 'get_input_embeddings')}"
    )

    if hasattr(model, "model"):
        logger.info(f"model.model type: {type(model.model)}")
        logger.info(
            f"model.model has 'get_input_embeddings': {hasattr(model.model, 'get_input_embeddings')}"
        )

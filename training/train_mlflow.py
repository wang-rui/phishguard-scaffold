...
from propagation.graph import (
    load_graph,
    greedy_minimize_spread,
    compute_propagation_loss,
)
from propagation.intervene import pick_candidates, evaluate_intervention_impact
...
class TrainingConfig:
    ...
    model_name_or_path: str = (
        "meta-llama/Llama-2-7b-hf"  # Primary LLaMA model as per research
    )
    ...
    def setup_model_and_data(self):
        ...
        try:
            self.model = PhishGuardClassifier(
                ...
                peft_cfg={"peft": self.config.peft, "lora_r": self.config.lora_r}
                if self.config.peft
                else None,
            )
        ...
        try:
            ...
            logger.info(
                f"Loaded graph with {self.graph.number_of_nodes()} nodes, {self.graph.number_of_edges()} edges"
            )
        except Exception as e:
            logger.warning(
                f"Could not load social graph, propagation loss will be disabled: {e}"
            )
            self.graph = None

        logger.info(
            f"Data loaded: {len(self.train_dataset)} train, {len(self.val_dataset)} val, {len(self.test_dataset)} test"
        )
        ...
        for step, batch in enumerate(pbar):
            ...
            with torch.cuda.amp.autocast(enabled=self.config.fp16):
                ...
                if self.graph is not None:
                    try:
                        risk_scores = torch.softmax(logits, dim=-1)[
                            :, 1
                        ...
                        signature=mlflow.models.signature.infer_signature(
                            {
                                "input_ids": torch.randint(
                                    0, 1000, (1, self.config.max_length)
                                ),
            ...
            if self.graph is not None:
                ...
                try:
                    ...
                    candidates = pick_candidates(
                        self.graph, risk_scores, self.config.topk_candidates
                    )
                    ...
                    mlflow.log_metrics(
                        {
                            ...
                            "intervention_effectiveness": impact.get(
                                "effectiveness", 0
                            ),
                        }
                    )

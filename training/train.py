...
from propagation.graph import load_graph, greedy_minimize_spread
from propagation.intervene import pick_candidates, risk_from_logits
...
class PhishGuardDataset(Dataset):
    ...
    def __init__(
        self, df, tokenizer, text_col, label_col, max_length, user_id_col="user_id"
    ):
...
def run(cfg_path: str, eval_only: bool = False):
    ...
    try:
        ...
        logger.info(
            f"Data split - Train: {len(split.train)}, Val: {len(split.val)}, Test: {len(split.test)}"
        )
    ...
    model = PhishGuardClassifier(
        cfg["model"]["model_name_or_path"], num_labels=2, peft_cfg=cfg["model"]
    )
            ...
            for batch in tqdm(train_loader, desc=f"Epoch {epoch + 1}"):
                ...
                try:
                    adv_loss = compute_adversarial_loss(
                        model, {"input_ids": input_ids, "attention_mask": attn}, cfg
                    )
                    adv_loss = safe_device_transfer(adv_loss, device)
                except Exception as e:
                    logger.warning(
                        f"Adversarial loss computation failed: {e}. Using zero loss."
                    )
                ...
                try:
                    ...
                    prop_loss = (
                        prop_probs.mean()
                    )  # Minimize average phishing probability
                except Exception as e:
                    logger.warning(
                        f"Propagation loss computation failed: {e}. Using zero loss."
                    )
                    prop_loss = torch.tensor(0.0, requires_grad=True, device=device)

                loss = (
                    cls_loss
                    + cfg["loss"]["lambda_adv"] * adv_loss
                    + cfg["loss"]["mu_prop"] * prop_loss
                )
                loss.backward()
                opt.step()
                sched.step()
    ...
    if os.path.exists(cfg["data"]["edges_csv"]):
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                user_ids.extend(
                    batch["user_id"]
                    if isinstance(batch["user_id"], list)
                    else [batch["user_id"]] * input_ids.size(0)
                )
        if len(logits_all):
            ...
            cand = pick_candidates(
                df_users, topk=int(cfg["propagation"]["topk_candidates"])
            )
            chosen = greedy_minimize_spread(
                G,
                budget=int(cfg["propagation"]["budget"]),
                risk=risk,
                candidates=cand,

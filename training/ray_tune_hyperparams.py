...
def train_phishguard_ray(config_dict, base_config, checkpoint_dir=None):
    ...
    try:
        ...
        optimizer = torch.optim.AdamW(
            trainer.model.parameters(), lr=config.lr, weight_decay=config.weight_decay
        )
...
def run_hyperparameter_optimization(
    ...
    run_config = ray.train.RunConfig(
        ...
        checkpoint_config=ray.train.CheckpointConfig(
            checkpoint_score_attribute="f1", checkpoint_score_order="max", num_to_keep=3
        ),
    )
    ...
    logger.info(
        f"Best trial final test F1: {best_trial.metrics.get('final_test_f1', 'N/A')}"
    )

    # Log best configuration to MLflow
    with mlflow.start_run(
        experiment_id=mlflow.get_experiment_by_name(
            "PhishGuard_Hyperparameter_Tuning"
        ).experiment_id
    ):
...
def distributed_training_example(config_path="configs/config.yaml"):
    ...
    def train_func(config_dict):
        ...
        config.run_name = (
            f"distributed_training_{ray.train.get_context().get_world_rank()}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Ray Tune hyperparameter optimization for PhishGuard"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        "--num-samples", type=int, default=20, help="Number of hyperparameter trials"
    )
    parser.add_argument(
        "--max-epochs", type=int, default=5, help="Maximum epochs per trial"
    )
    parser.add_argument(
        "--gpus-per-trial", type=float, default=0.25, help="GPU fraction per trial"
    )
    parser.add_argument(
        "--distributed", action="store_true", help="Run distributed training example"
    )
    ...
    if args.distributed:
        ...
        logger.info(
            f"Best configuration achieved F1 score: {best_trial.metrics['f1']:.4f}"
        )

...
def remove_duplicates(df: pd.DataFrame, text_col: str) -> pd.DataFrame:
    ...
    final_count = len(df)
    logger.info(
        f"Removed {initial_count - final_count} duplicates ({initial_count} -> {final_count})"
    )
...
def filter_by_length(
    df: pd.DataFrame, text_col: str, min_length: int = 10, max_length: int = 512
) -> pd.DataFrame:
    ...
    # Filter by length
    df = df[
        (df[text_col].str.len() >= min_length) & (df[text_col].str.len() <= max_length)
    ]

    final_count = len(df)
    logger.info(
        f"Filtered by length ({min_length}-{max_length} chars): {initial_count} -> {final_count}"
    )
...
def load_and_split(tweets_csv: str, cfg: Dict) -> SplitData:
    ...
    if len(df) < 100:
        logger.warning(
            f"Very few samples after preprocessing ({len(df)}). Consider adjusting filters."
        )
    ...
    try:
        train_df, temp_df = train_test_split(
            df, test_size=(1 - train_size), stratify=df[label_col], random_state=42
        )
        ...
        val_rel = val_size / (val_size + test_size)
        val_df, test_df = train_test_split(
            temp_df, test_size=(1 - val_rel), random_state=42
        )

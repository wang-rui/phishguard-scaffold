...
from training.constants import (
    INFLUENCE_DECAY_FACTOR, MIN_EDGE_WEIGHT, MAX_EDGE_WEIGHT,
    HIGH_RISK_SEED_LIMIT, HIGH_RISK_THRESHOLD, RISK_BIAS_MULTIPLIER,
    PAGERANK_WEIGHT, BETWEENNESS_WEIGHT, OUT_DEGREE_WEIGHT
)
    ...
    if edges_df is not None:
        for _, row in edges_df.iterrows():
            ...
            if weight >= edge_weight_threshold:
                G.add_edge(src, dst, weight=max(MIN_EDGE_WEIGHT, min(MAX_EDGE_WEIGHT, weight)), edge_type="explicit")
...
def load_graph(edges_csv: str) -> nx.DiGraph:
    ...
    for _, row in df.iterrows():
        ...
        G.add_edge(row["src"], row["dst"], weight=max(MIN_EDGE_WEIGHT, min(MAX_EDGE_WEIGHT, p)))
    return G

def ic_spread(G: nx.DiGraph, seeds: List[str], samples: int = 100, max_steps: int = 10) -> float:
...
def compute_influence_scores(G: nx.DiGraph, user_risk: Dict[str, float]) -> Dict[str, float]:
    ...
    for node in G.nodes:
        ...
        network_influence = (PAGERANK_WEIGHT * norm_pagerank + 
...
def greedy_minimize_spread(G: nx.DiGraph, budget: int, risk: Dict[str, float], candidates: List[str], samples: int = 100) -> List[str]:
    ...
    high_risk_seeds = [u for u in cand[:min(HIGH_RISK_SEED_LIMIT, len(cand))] if risk.get(u, 0.0) > HIGH_RISK_THRESHOLD]
    ...
    for iteration in range(min(budget, len(cand))):
        ...
        for u in cand:
            ...
            neighbors = list(G_working.predecessors(u)) + list(G_working.successors(u))
            edges_to_restore = [(pred, u, G_working[pred][u]) for pred in G_working.predecessors(u) if G_working.has_edge(pred, u)]
            edges_to_restore += [(u, succ, G_working[u][succ]) for succ in G_working.successors(u) if G_working.has_edge(u, succ)]
            
            # Remove node temporarily
            G_working.remove_node(u)
            
            # Estimate spread with intervention
            remaining_seeds = [s for s in high_risk_seeds if s != u and s in G_working.nodes]
            spread_with_intervention = ic_spread(G_working, remaining_seeds, samples) if remaining_seeds else 0.0
            ...
            risk_bias = 1.0 + RISK_BIAS_MULTIPLIER * risk.get(u, 0.0)  # Higher weight for risky nodes

...
def risk_from_logits(user_ids: List[str], logits: torch.Tensor) -> Dict[str, float]:
    ...
    for user_id, risk_scores in user_risks.items():
        if len(risk_scores) == 1:
            ...
            final_risks[user_id] = (
                0.6 * mean_risk + 0.3 * max_risk + 0.1 * consistency * mean_risk
            )

    return final_risks


def compute_user_influence_metrics(
    G: nx.DiGraph, user_ids: List[str]
...
def advanced_risk_assessment(
    ...
    logits: torch.Tensor,
    G: nx.DiGraph,
    tweets_df: Optional[pd.DataFrame] = None,
) -> Dict[str, float]:
    ...
    if tweets_df is not None and len(tweets_df) > 0:
        for user_id in base_risks.keys():
            ...
            url_ratio = (
                user_tweets["url"].notna().mean()
                if "url" in user_tweets.columns
                else 0.0
            )
            ...
            behavioral_scores[user_id] = (
                0.5 * url_ratio
                + 0.3 * min(1.0, activity_freq / 5.0)
                + 0.2 * len(user_tweets) / 100.0
            )
    else:
    ...
    for user_id in base_risks.keys():
        ...
        final_risk = (
            0.5 * base_risk  # Model prediction weight
...
def pick_candidates(
    ...
    for user_id in activity_counts.index:
        ...
        activity_score = (
            activity_counts[user_id] / activity_counts.iloc[0]
        ...
        if G is not None and user_id in G.nodes:
            try:
                ...
                degree_centrality = (
                    G.degree(user_id, weight="weight") / (len(G.nodes) - 1)
                    if len(G.nodes) > 1
                    else 0.0
                )
...
def evaluate_intervention_impact(
    ...
    high_risk_users = [
        u for u, risk in risk_scores.items() if risk > 0.5 and u in G.nodes
    ]
    ...
    intervened_spread = (
        ic_spread(G_intervened, remaining_seeds, samples) if remaining_seeds else 0.0
    )

    # Calculate impact metrics
    spread_reduction = baseline_spread - intervened_spread
    relative_reduction = (
        spread_reduction / baseline_spread if baseline_spread > 0 else 0.0
    )

    # Cost-effectiveness (spread reduction per intervention node)
    cost_effectiveness = (
        spread_reduction / len(intervention_nodes)

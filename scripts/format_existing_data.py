...
class DataFormatter:
    ...
    def standardize_timestamp(self, df: pd.DataFrame, time_col: str) -> pd.DataFrame:
        ...
        try:
            ...
            df[time_col] = pd.date_range(
                start=start_date, periods=len(df), freq="H"
    ...
    def format_dataset(
        self, input_path: str, output_path: str, column_overrides: Optional[Dict] = None
    ) -> None:
        ...
        if detected["time"]:
            ...
            output_df["timestamp"] = pd.date_range(
                start=start_date, periods=len(output_df), freq="H"
            ).strftime("%Y-%m-%dT%H:%M:%SZ")
        ...
        output_df = output_df[
            (output_df["text"].str.len() >= 5) & (output_df["label"].isin([0, 1]))
        ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Format existing Twitter dataset for PhishGuard"
    )
    parser.add_argument("--input", "-i", required=True, help="Input CSV file path")
    parser.add_argument(
        "--output", "-o", default="data/tweets.csv", help="Output CSV file path"
    )
    ...
    parser.add_argument(
        "--time-col", help="Timestamp column name (if not auto-detected)"
    )
    ...
    try:
        ...
        logger.info(
            "You can now run: python -m training.train --config configs/config.yaml"
        )
    except Exception as e:

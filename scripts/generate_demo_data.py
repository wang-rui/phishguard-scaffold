...
class PhishingDataGenerator:
    ...
    def generate_tweets(self, users: List[dict], num_tweets: int) -> pd.DataFrame:
        ...
        for i in range(num_tweets):
            ...
            medium_activity_users = [
                u for u in users if u["activity_level"] == "medium"
            ]
    ...
    def generate_social_graph(
        self, users: List[dict], tweets_df: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        for user in users:
            ...
            followed_users = random.sample(
                [u for u in users if u["user_id"] != user["user_id"]], num_follows
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate demo Twitter data for PhishGuard"
    )
    parser.add_argument(
        "--tweets", "-t", type=int, default=5000, help="Number of tweets to generate"
    )
    parser.add_argument(
        "--users", "-u", type=int, default=1000, help="Number of users to generate"
    )
    ...
    logger.info(
        f"Generating demo data with {args.tweets} tweets and {args.users} users"
    )
    ...
    logger.info(
        f"Class distribution: Legitimate: {class_dist[0]}, Phishing: {class_dist[1]}"
    )

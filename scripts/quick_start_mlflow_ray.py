    ...
    try:
        result = subprocess.run(
            [
                "python",
                "-m",
                "training.ray_tune_hyperparams",
                "--config",
                "configs/mlflow_config.yaml",
                "--num-samples",
                "5",  # Small number for quick start
                "--max-epochs",
                "3",
                "--gpus-per-trial",
                "0.25",
...
def main():
    ...
    parser.add_argument(
        "--setup", action="store_true", help="Setup environment and check dependencies"
    )
    ...
    parser.add_argument("--start-ui", action="store_true", help="Start MLflow UI")
    ...
    if not any(vars(args).values()):
        logger.info(
            "No action specified. Use --help for options or --help-detailed for examples"
        )
        return

...
class MLflowExperimentRunner:
    ...
    def run_single_experiment(
        self, config: Dict[str, Any], experiment_name: str, run_name: str
    ) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.error(
                    f"❌ Experiment {run_name} failed with return code {result.returncode}"
                )
    ...
    def run_experiments(
        ...
        experiment_name: str = "PhishGuard_GridSearch",
    ) -> List[str]:
        ...
        for i, config in enumerate(configs):
            ...
            detailed_run_name = f"{run_name}_" + "_".join(
                param_info[:3]
            )  # Limit length

            success = self.run_single_experiment(
                config, experiment_name, detailed_run_name
            )
...
def run_learning_rate_experiments():
    ...
    return runner.run_experiments(
        param_grid=param_grid, experiment_name="PhishGuard_LearningRate_Study"
    )
...
def run_loss_weights_experiments():
    ...
    return runner.run_experiments(
        param_grid=param_grid, experiment_name="PhishGuard_LossWeights_Study"
    )
...
def run_adversarial_experiments():
    ...
    return runner.run_experiments(
        param_grid=param_grid, experiment_name="PhishGuard_Adversarial_Study"
    )
...
def run_model_architecture_experiments():
    ...
    return runner.run_experiments(
        param_grid=param_grid, experiment_name="PhishGuard_Architecture_Study"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run MLflow experiments for PhishGuard"
    )
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/mlflow_config.yaml",
    ...
    parser.add_argument(
        "--custom-grid", type=str, help="Path to custom parameter grid YAML file"
    )

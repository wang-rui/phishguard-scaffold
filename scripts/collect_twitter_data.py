...
class TwitterDataCollector:
    ...
    def collect_user_interactions(
        self, user_ids: List[str], max_users: int = 100
    ) -> List[Dict]:
        """Collect user interaction data for social graph construction."""
        edges = []

        logger.info(
            f"Collecting interactions for {min(len(user_ids), max_users)} users..."
        )
                        ...
                        if tweet.referenced_tweets:
                            for ref in tweet.referenced_tweets:
                                if ref.type in ["retweeted", "replied_to"]:
                                    ...
                                    try:
                                        ref_tweet = self.client.get_tweet(
                                            ref.id, expansions=["author_id"]
                                        )
                                        if (
                                            ref_tweet.includes
                                            and "users" in ref_tweet.includes
                                        ):
                                            parent_user_id = str(
                                                ref_tweet.includes["users"][0].id
                                            )

                                            # Calculate interaction weight based on relationship
                                            weight = (
                                                0.1 if ref.type == "retweeted" else 0.05
                                            )
